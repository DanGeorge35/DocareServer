...
class AuthenticationController {
  ...
  static async resetpassword (req: any, res: any): Promise<any> {
    try {
      ...
      res.status(200).json({ success: true, message: 'Your account reset code has been sent to your email!' })
      await SendMail({
  ...
  static async changepasswordreset (req: any, res: any): Promise<any> {
    try {
      ...
      if (account.Token !== data.resetcode) {
        const result: any = {
          success: false,
          message: 'Invalid Reset Code, Kindly check your email INBOX / SPAM for your account reset code!',
          code: 400
        }
        return res.status(result.code).json(result)
      }

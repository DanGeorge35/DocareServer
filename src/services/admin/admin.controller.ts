...
class AdminController {
  static async login(req: any, res: any, next: any): Promise<any> {
    try {
      ...
      data.invChart = await sequelize.query(
        "SELECT DATE_FORMAT(createdAt, '%b, %Y') AS MonthPeriod,SUM(CAST(Amount AS DECIMAL(10,2))) AS TotalAmount FROM investments where Status = 'Active' GROUP BY MonthPeriod order by id",
  ...
  static async getSingleAdmin(req: any, res: any, next: any): Promise<any> {
    try {
      ...
      if (!singleAdmin) {
        return res
          .status(400)
          .json({ success: false, data: `No Admin with the id ${req.params.id}` });
      }
  ...
  static async datarecord(req: any, res: any, next: any): Promise<any> {
    try {
      const data: any = {};
      data.Investments = await sequelize.query(
        'SELECT investors.FullName, investors.Phone,  investors.Email, investors.UserID,investments.Amount,investments.Duration,investments.Status,investors.BankName,investors.AccountNumber,investors.AccountName FROM investments INNER JOIN investors ON investments.investorId = investors.UserID',
        { type: QueryTypes.SELECT }
      );
      data.Investors = await sequelize.query('SELECT * FROM `investors` ', {
        type: QueryTypes.SELECT
      });
      data.Referrals = await sequelize.query('SELECT * FROM `referrals` ', {
        type: QueryTypes.SELECT
      });
  ...
  static async updateAdmin(req: any, res: any, next: any): Promise<any> {
    try {
      ...
      return res
        .status(200)
        .json({ success: true, data: dadmin, message: 'Admin information updated' });
    } catch (error: any) {
      const err = { success: false, code: 400, message: `SYSTEM ERROR : ${error.message}` };
      console.error(error);
      return res.status(400).send(err);
